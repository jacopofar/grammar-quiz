...
def test_get_languages(client):
    ...
    assert 'iso693_3' in languages['languages'][0]
    # the codes are always 3 letters
    assert len(languages['languages'][0]['iso693_3']) == 3
    assert 'name' in languages['languages'][0]
...
def test_draw_cards(client):
    ...
    assert 'to_text' in card
    # at least an ideogram or a letter!
    assert len(card['to_text']) > 1
    assert 'from_text' in card
    # at least an ideogram or a letter!
    assert len(card['from_text']) > 1
...
def test_answer_then_redraw(client):
    ...
    # they are, because the test user is anonymous and so is stateless
    # # the answers we already sent are not in the new selection
    assert (card1['from_id'], card1['to_id']) in cards
    assert (card2['from_id'], card2['to_id']) in cards
